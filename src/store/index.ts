...
export const store = createStore<State>({
  ...
  mutations: {
    ...
    [ADD_CHILD](state: State, payload: { component: Component, parentId: string }) {
      ...
      if (!parent) { return }
      parent.children.push(payload.component)
    },
  ...
  plugins: [
    (store) => {
      store.subscribe(mutation => {
        if (![ADD_CHILD, SET_PREVIEW_SIZE, FLEX_SET_DIRECTION, FLEX_SET_SPACING, SOURCE_SET_SOURCE, FLEX_SET_DISTRIBUTION].includes(mutation.type)) {
          return;
        }
